    ...
    .map(
      ...
      <tr data-guia="${paquete.numeroGuia}">
        ...
        <td>
          ...
          <button class="btn-accion btn-eliminar" data-action="eliminar" type="button">
            Eliminar
          </button>
        </td>
      </tr>
...
async function eliminarPaquete(guia) {
  try {
    const respuesta = await fetch(`/api/paquetes/${encodeURIComponent(guia)}`, {
      method: "DELETE",
    });
    await parsearRespuesta(respuesta);
    mostrarMensaje("mensaje-admin", `Paquete ${guia} eliminado correctamente.`, "ok");
...
function mostrarModalConfirmacion(guia) {
  ...
  modal.innerHTML = `
    <div class="modal-backdrop">
      <div class="modal-caja">
        <p>¿Seguro que deseas eliminar el paquete <strong>${guia}</strong>?</p>
        <div class="modal-botones">
          <button id="modal-confirmar" class="btn btn-peligro" type="button">Sí, eliminar</button>
          <button id="modal-cancelar" class="btn btn-secundario" type="button">Cancelar</button>
        </div>
      </div>
    </div>
  `;
  ...
  if (pagina === "admin") {
    ...
    if (tabla) {
      tabla.addEventListener("click", (evento) => {
        const botonActualizar = evento.target.closest("button[data-action='guardar-estado']");
        if (botonActualizar) {
          const fila = botonActualizar.closest("tr");
          if (fila) actualizarEstadoDesdeFila(fila);
          return;
        }

        const botonEliminar = evento.target.closest("button[data-action='eliminar']");
        if (botonEliminar) {
          const fila = botonEliminar.closest("tr");
          if (fila) mostrarModalConfirmacion(fila.dataset.guia);
        }
      });
    }
